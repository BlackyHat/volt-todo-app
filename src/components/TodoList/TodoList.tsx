...
export const TodoList: React.FC = () => {
  ...
  return tasks.length === 0 ? <p className="text-base font-medium
  text-slate-800 md:text-2xl">
      Try to add some tasks ...
  </p> : <ul className="xl:grid- grid gap-3 md:grid-cols-2 md:gap-6
  xl:row-span-full xl:grid-cols-3">
    {tasks.map((task) => <li
      key={task.id}
    ...
    >
      ...
      justify-center gap-2">
        <span
          className={`rounded-xl border border-slate-100 p-1 px-2 
          text-sm uppercase ${
  task.completed ? 'bg-green-100' : 'bg-orange-100'
        ...
        <button
          ...
          focus-within:text-red-800 hover:text-red-800"
      ...
      <button
        ...
        className={`p-2 text-base font-medium text-slate-800 md:text-2xl ${
      ...
      <p
        className={`whitespace-pre-line px-2 text-sm font-light 
        text-slate-600 md:text-lg ${
  task.completed ? 'line-through' : 'no-underline'
}`}
      >
        {task.description}
